...
//    } catch (e) {
//      // if bridging isn't present, silently drop its
//    }
//  };
...
// swap_script.js
// --- Globals injected by Dart: solanaWeb3, solanaAgentKit, bs58
// --- Functions exposed: initializeDependencies, executeSwapWrapper


// ===== begin swap_script.js =====
// Ensure sendMessage → postMessage bridge
if (typeof sendMessage === 'undefined') {
  this.sendMessage = (channel, message) => {
    try {
      postMessage(channel, message);
    } catch (e) {
      console.error('[SWAP] postMessage failed:', e);
    }
  };
}

// … the rest of your file follows …
// Health check utility
async function checkRpcHealth(rpcUrl) {
  console.log(`[RPC] Skipping health check for now`);
  return true;
}
...
//    console.log(`[RPC] Health check result: ${JSON.stringify(json)}`);
//    return json.result === 'ok';
//  } catch (err) {
//    console.log(`[RPC] Health check failed: ${err}`);
//    return false;
//  }
//}
