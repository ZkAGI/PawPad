...
class SignalSchedulerService {
  Timer? _bitcoinTimer;
  Timer? _autonomousTimer;
  Timer? _customStrategyTimer; // Add this line
  bool _isInitialized = false;
  ...
  void _setupScheduledTimers(AgentProvider agentProvider, BuildContext context) {
    ...
    // Check Autonomous trading signal every hour (to ensure we don't miss the 12-hour window)
    // Changed comment to reflect new 12-hour window
    _autonomousTimer = Timer.periodic(const Duration(hours: 1), (timer) {
      _checkAutonomousSignal(agentProvider, context);
    });

    // Check Custom Strategy signal every hour (to ensure we don't miss the 6-hour window)
    _customStrategyTimer = Timer.periodic(const Duration(hours: 1), (timer) {
      _checkCustomStrategySignal(agentProvider, context);
    });
  }
  ...
  Future<void> _checkAutonomousSignal(AgentProvider agentProvider, BuildContext context) async {
    ...
    try {
      // Check if we should check autonomous signal now (every 12 hours) - changed from 6 to 12
      final shouldCheck = await agentProvider.shouldCheckAutonomousSignal();
  ...
  // Add method to check Custom Strategy signal (every 8 hours)
  Future<void> _checkCustomStrategySignal(AgentProvider agentProvider, BuildContext context) async {
    if (!context.mounted) return;

    try {
      // Check if we should check custom strategy signal now (every 8 hours)
      final shouldCheck = await agentProvider.shouldCheckCustomStrategySignal();
      ...
      if (shouldCheck) {
        ...
        final result = await agentProvider.checkCustomStrategySignal(balance);
      ...
      print('Error checking Custom Strategy signal: $e');
    }
  }
  ...
  void dispose() {
    _bitcoinTimer?.cancel();
    _autonomousTimer?.cancel();
    _customStrategyTimer?.cancel(); // Add this line
    _isInitialized = false;
  }
}